...
const sequenceList = (content) => {
  ...
  const newData = [...dataStore.flowerData]
  if (Num.value === 1) {
    txt.value = content + '大到小'
    newData.sort((a, b) => b[content] - a[content])
    updateShowData(newData, 'flo')
  } else if (Num.value === 2) {
...
const TotalPage = computed(() => {
  return Math.ceil(dataStore.flowerData.length / itemShowNum.value)
})
//監聽資料並回到第一頁
watch(
  () => dataStore.flowerData,
  () => {
    currentPage.value = 1
  }
)
...
const showPageList = computed(() => {
  ...
  return dataStore.flowerData.slice(start, end)
})
//點擊頁數導航
...
const noSearched = computed(() => {
  if (dataStore.dataIsSerached && dataStore.flowerData.length === 0) {
    return true
  }
  return false
})
    ...
    <table class="table table-hover">
      ...
      <tbody>
        ...
        <tr v-show="noSearched">
          <td colspan="7" class="text-center py-20">查詢不到花朵當日的交易資訊QQ</td>
        </tr>
